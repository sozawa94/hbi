program main
  ...
  case('two')
   ...
   xel(2000+i)=cos(angle)*ds*(i-1)+3.0
     ...
     yel(2000+i)=sin(angle)*ds*(i-1)-0.0001d0
     yer(2000+i)=sin(angle)*ds*i-0.0001d0
  ...
  !write(*,*) xr(1:n/4),yr(1:n/4)
    ...
    do j=1,jmax
      ...
      do i=1,j-1
        ...
        dx1=ds*ns(j)*cos(ang(j))*1.1d0
        dx2=ds*ns(i)*cos(ang(i))*1.1d0
        dxa=xg(i)+ns(i)*0.5d0*ds*cos(ang(i))-(xc-0.5d0*ds*cos(ang(j)))

        dy1=ds*ns(j)*sin(ang(j))*1.1d0
        dy2=ds*ns(i)*sin(ang(i))*1.1d0
