program main
 ...
 counts=10
  ...
  !no restart
    ...
    !computing flow rate
    qflow(1)=-kp(1)/eta*1e3*(pf(2)-pfhyd(2)-pf(1)+pfhyd(1))/ds0
    do i=2,ncell-1
      qflow(i)=-kp(i)/eta*1e3*(pf(i+1)-pfhyd(i+1)-pf(i-1)+pfhyd(i-1))/2/ds0
    end do
    qflow(ncell)=-kp(ncell)/eta*1e3*(pf(ncell)-pfhyd(ncell)-pf(ncell-1)+pfhyd(ncell-1))/ds0
  ...
  !Figure 10 of the fault-valve paper
  !tout=360d0*365*24*60*60
...
contains
  ...
  subroutine initcond_thrust()
    ...
    do i=1,ncellg
      ...
      !Zhu et al. (2020)
      ! a(i)=0.01+ycol(i)*0.00133
      ! b(i)=a(i)+0.01
      ! if(ycol(i)>15) then
      !   a(i)=0.03+(ycol(i)-15)*0.003
      !   b(i)=0.04
      ! end if
  ...
  subroutine initcond_swarm()
    implicit none
    real(8),parameter::g=9.80,rhor=2.67d3,rhof=1.0d3,ofs=5.0
  ...
  subroutine implicitsolver(pf,sigma,ks,h,time,dtnxt)
    ...
    if(outpertime) then
      dtnxt=min(dtnxt,dtout*365*24*3600)
    end if
    return
  end subroutine
  ...
  subroutine Beuler(pf,cdiff,h,pfnew,time,niter)
    ...
    if(setting=='thrust') then
      do i=3,Ncellg-2
        SAT(i)=-qin(i)/beta/phi(i)*1e-9*h/ds0
      end do
      !SAT(1200)=9*q0/beta/phi(1200)*1e-9*h/ds0
  ...
  subroutine derivs(x, y, dydx)
    ...
    do i = 1, NCELL
      ...
      sigmaetmp(i) = y(4*i-1)
      kstmp(i)=y(4*i)
      veltmp(i) = 2*vref*dexp(-psitmp(i)/a(i))*dsinh(tautmp(i)/sigmaetmp(i)/a(i))
    ...
    do i=1,NCELL
      ...
      case('aging')
        if(b(i)==0) then
          ...
          dpsidt=b(i)/dc(i)*vref*dexp((f0(i)-psitmp(i))/b(i))-b(i)*veltmp(i)/dc(i)
        end if
      !slip law
      case('slip')
      ...
      dvdsig=-2*vref*dexp(-psitmp(i)/a(i))*dcosh(tautmp(i)/sigmaetmp(i)/a(i))*tautmp(i)/(a(i)*sigmaetmp(i)**2)
      dvdpsi=-veltmp(i)/a(i)
  ...
  subroutine foward_check()
    ...
    ! case('3dnt','3dht','3dnr','3dhr')
      ...
      if(my_rank==0) then
        do i=1,NCELL
          i_=st_sum%lodc(i)
          write(29,'(6e16.4)') xcol(i_),ycol(i_),zcol(i_),disp(i_),ret1(i),ret2(i)
...
subroutine output_BP6()
  ...
  !   &qflow(locid(i)),log10(dc(locid(i))/vref*exp((psi(locid(i))-f0(locid(i)))/b(locid(i))))
  ! end do
  write(40,'(e22.14,4e15.7)') x,log10(maxval(vel)),sum(vel)*rigid*1e9*ds0*1e3,pf(ncellg/2),log10(vel(ncellg/2))
