program main
  ...
  real(8),allocatable::xcol(:),ycol(:),zcol(:),phi(:),phiG(:)
  ...
  do k=1,NSTEP1
    ...
    if(my_rank.eq.0) then
      time2= MPI_Wtime()
      write(52,'(i6,f18.4,4e16.5,f16.4)')k,x,maxval(log10(abs(velG))),sum(dispG)/NCELLg,sum(muG)/NCELLg,sum(effG)/NCELLg,time2-time1
      !do i=1,size(vmax)
