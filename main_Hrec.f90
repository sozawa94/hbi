program main
  ...
  real(8)::vc0,mu0,dtinit,onset_time,tr,vw0,fw0
  ...
  real(8),allocatable::vel(:),tau(:),sigma(:),disp(:),mu(:),s(:),sg(:),eff(:)
  real(8),allocatable::velG(:),tauG(:),sigmaG(:),dispG(:),muG(:),acG(:),idisp(:),effG(:)
  ...
  read(33,*) dum,tr !viscoelastic relaxation
  ...
  read(33,*) dum,eps !error allowance in time integration
  !read(*,*) amp
  !read(*,*) omega
  ...
  allocate(vel(NCELL),tau(NCELL),sigma(NCELL),mu(NCELL),s(NCELL),disp(NCELL),ac(NCELL),eff(NCELL))
  allocate(velG(NCELLg),tauG(NCELLg),sigmaG(NCELLg),dispG(NCELLg),muG(NCELLg),sg(NCELLg),acG(NCELLg),idisp(NCELLg),effG(NCELLg))
  select case(problem)
  case('2dp','2dpv')
  ...
  do i=1,NCELL
    ...
    s(i)=omega
    theta=s(i)/velinit
    mu(i)=mu0+(a(i)-b(i))*dlog(velinit/vref)
    vel(i)=velinit
    sigma(i)=sigma0
    tau(i)=mu(i)*sigma(i)
  end do
  ...
  do k=1,NSTEP1
    ...
    Call MPI_BARRIER(MPI_COMM_WORLD,ierr)
    call MPI_ALLGATHERv(sigma,NCELL,MPI_REAL8,sigmaG,rcounts,displs,                &
    &     MPI_REAL8,MPI_COMM_WORLD,ierr)
    call MPI_ALLGATHERv(tau,NCELL,MPI_REAL8,tauG,rcounts,displs,                &
    &     MPI_REAL8,MPI_COMM_WORLD,ierr)
    ...
    & MPI_REAL8,MPI_COMM_WORLD,ierr)
    call MPI_ALLGATHERv(eff,NCELL,MPI_REAL8,effG,rcounts,displs,&
    & MPI_REAL8,MPI_COMM_WORLD,ierr)
    call MPI_ALLGATHERv(ac,NCELL,MPI_REAL8,acG,rcounts,displs,                &
    &     MPI_REAL8,MPI_COMM_WORLD,ierr)
    ...
    if(my_rank.eq.0) then
      time2= MPI_Wtime()
      write(52,'(i6,f18.4,4e16.5,f16.4)')k,x,maxval(log10(velG)),sum(dispG)/NCELLg,sum(muG)/NCELLg,sum(effG)/NCELLg,time2-time1
      !do i=1,size(vmax)
      ...
      if(outfield) then
        ...
        case('2dp','2dn')
          do i=1,NCELLg
            write(50,'(9e15.6,i6)') xcol(i),ycol(i),log10(velG(i)),tauG(i),sigmaG(i),muG(i),dispG(i),sG(i),x,k
          end do
          write(50,*)
        end select
      end if
    ...
    if(abs(maxval(velG)).gt.velmax) then
      if(my_rank .eq. 0) write(*,*) 'slip rate exceeds threshold'
      exit
    end if
...
contains
  ...
  subroutine coordinate2dm(geofile,NCELLg,xel,xer,yel,yer,xcol,ycol,ang)
    implicit none
    integer,intent(in)::NCELLg
    character(128),intent(in)::geofile
    real(8),intent(out)::xel(:),xer(:),yel(:),yer(:),xcol(:),ycol(:),ang(:)
    ...
    real(8)::dx,nx(NCELLg),ny(NCELLg),r(NCELLg)
    real(8),allocatable::xr(:,:),yr(:,:)

    !reading geometry
    open(30,file=geoinfo)
    read(30,*) segment
    allocate(ne(segment))
    read(30,*) ne
    close(30)
    allocate(xr(segment,maxval(ne)),yr(segment,maxval(ne)))

    open(20,file=geofile)
    do k=1,segment
      do i=0,ne(k)
        read(20,*) xr(k,i),yr(k,i)
      end do
      read(20,*)
    end do
    close(20)
  ...
  subroutine derivs(x, y, dydx)!,,st_leafmtxp,st_bemv,st_ctl)
    ...
    real(8) :: veltmp(NCELL),tautmp(NCELL),sigmatmp(NCELL),efftmp(NCELL)
    real(8) :: dlnvdt(NCELL),dtaudt(NCELL),dsigdt(NCELL),deffdt(NCELL)
    ...
    case('d')
      call deriv_d(sum_gs,sum_gn,veltmp,tautmp,sigmatmp,dlnvdt,dtaudt,dsigdt)
  ...
  subroutine deriv_d(sum_gs,sum_gn,veltmp,tautmp,sigmatmp,dlnvdt,dtaudt,dsigdt)
    implicit none
    integer::i
